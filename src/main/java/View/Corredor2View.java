...
public class Corredor2View extends Application implements Observador {
    ...
    private void loopDoJogo(long tempoAtualNano) {
        ...
        if (playerHitbox.getBoundsInParent().intersects(blocoGato.getBoundsInParent())) {
            if (!emDialogo) {
                ...
                int carinho = controller.carinho();
                if(carinho == 1){
                    textoDialogo.setText("Gatinho: Miau! \n\nPerde energia\nGanha motivação");
                }else if (carinho == 2){
                    textoDialogo.setText("Luiza: Ouch!!! O gato me mordeu\n\nPerde Saude, Energia e Motivação.");
                }else if (carinho == 0){
                    textoDialogo.setText("Energia inuficiente.");
                }
            }
        } else {
            if (emDialogo) {
    ...
    public void start(Stage primaryStage) {
        ...
        Image imagemGato = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/gato.gif")));
        gatoView = new ImageView(imagemGato);
        gatoView.setLayoutX(1200.0);
        gatoView.setLayoutY(230.0);
        mundoBox.getChildren().add(gatoView);
        ...
        criarObstaculo(1205.0, 260.0, 50.0, 35.0, mundoBox);
        ...
        criarObstaculo(22.0, 277.0, 52.0, 82.0, mundoBox);
